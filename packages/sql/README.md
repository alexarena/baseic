...
`@baseic/sql` doesn't execute SQL — it just generates it. The intended use case is taking data from a spreadsheet (or some other tabular source), defining a small set of tables with [zod](https://zod.dev) schemas, and emitting the `CREATE TABLE` + `INSERT` statements you'd run against a fresh SQLite database to produce a one-off report.
...
`@baseic/sql` builds with [tsdown](https://tsdown.dev):
